...
In this step the mean and median of the total number of steps per day are 
calculated.
...
`steps` are equal to NA and therefore no mean or median can be calculated. Note
...
Therefore all 2,304 missing values are replaced by the mean value of the
5-minute intervals.
...
the absence of the high frequency in the leftmost histogram bin. The
...
the days for which all 288 `steps` values are equal to NA in the non-imputed
...
and in the weekends. While the weekday pattern shows a large peak in the morning
